...
export const initAccountAuth = async locale => {
  ...
  const loadProfile = async user => {
    const [profileResult, settingsResult] = await Promise.all([
      ...
      supabase
        ...
        .select('locale, age_confirmed_at, privacy_acknowledged_at, registration_completed_at')
        .eq('user_id', user.id)
        .single(),
    ]);
  ...
  const populateUser = (prefix, user, profile) => {
    const name = profile?.display_name || user.user_metadata?.full_name || user.user_metadata?.name || user.email || '';
    const avatarUrl = profile?.avatar_url || user.user_metadata?.avatar_url || user.user_metadata?.picture;
    const nameElement = modal.querySelector(`[data-${prefix}-name]`);
    const emailElement = modal.querySelector(`[data-${prefix}-email]`);
    const avatar = modal.querySelector(`[data-${prefix}-avatar]`);
  ...
  const prepareGoogle = async config => {
    ...
    if (!currentUser) {
      window.setTimeout(() => google.accounts.id.prompt(), 1200);
    }
  };
  ...
  control.addEventListener('click', () => {
    if (currentUser) {
      ...
      openModal(currentProfile?.registration_completed_at ? 'profile' : 'completion');
      return;
    }
    openModal('signin');
  });
  ...
  modal.querySelector('[data-account-signout]').addEventListener('click', async () => {
