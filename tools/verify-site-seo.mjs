...
for (const absolute of files) {
  ...
  const expectedLang = urlPath.startsWith('/de/') ? 'de' : urlPath.startsWith('/es/') ? 'es' : urlPath.startsWith('/it/') ? 'it' : urlPath.startsWith('/pl/') ? 'pl' : urlPath.startsWith('/uk/') ? 'uk' : urlPath.startsWith('/pt/') ? 'pt' : urlPath.startsWith('/fr/') ? 'fr' : 'en';
...
for (const [urlPath, page] of pages) {
  const basePath = urlPath.replace(/^\/(?:de|es|it|pl|uk|pt|fr)(?=\/)/, '');
  const localePaths = {
    en: basePath,
  ...
  for (const [locale, variantPath] of Object.entries(localePaths)) {
    ...
    const pageLocale = urlPath.startsWith('/de/') ? 'de' : urlPath.startsWith('/es/') ? 'es' : urlPath.startsWith('/it/') ? 'it' : urlPath.startsWith('/pl/') ? 'pl' : urlPath.startsWith('/uk/') ? 'uk' : urlPath.startsWith('/pt/') ? 'pt' : urlPath.startsWith('/fr/') ? 'fr' : 'en';
