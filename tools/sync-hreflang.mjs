...
const walk = directory => {
  for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
    if (['.git', 'node_modules', 'tools', 'de', 'es', 'it', 'pl', 'uk', 'pt', 'fr', 'hi', 'fi'].includes(entry.name) && directory === root) continue;
...
for (const sourceFile of sourceFiles) {
  ...
  const alternates = [
    ...locales.map(locale =>
      `<link rel="alternate" hreflang="${locale === 'fi' ? 'fi-FI' : locale === 'hi' ? 'hi' : `${locale}${suffix}`}" href="${origin}${localizedRoute(route, locale)}" />`
    ),
